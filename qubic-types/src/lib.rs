...
fn addcarry_u64(c_in: u8, a: u64, b: u64, out: &mut u64) -> u8  {
    ...
    {
        let mut c_out = a.overflowing_add(b);
...
fn subborrow_u64(b_in: u8, a: u64, b: u64, out: &mut u64) -> u8 {
    ...
    #[cfg(not(target_arch = "x86_64"))]
    {
        let mut b_out = a.overflowing_sub(b);
...
impl QubicId {
    ...
    pub fn verify(&self, message_digest: [u8; 32], signature: [u8; 64]) -> bool {
...
impl QubicWallet {
    ...
    pub fn sign<T: Sized>(&self, message: &T) -> Signature {
