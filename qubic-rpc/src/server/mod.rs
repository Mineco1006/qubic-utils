...
mod tests {
    ...
    use crate::{
        qubic_rpc_types::{
            BlockHeightResponse, ComputorsWrapper, LatestTick, TickInfoWrapper, WalletBalance,
        },
        server::{qubic_rpc_router_v2, RPCState},
    };
    ...
    async fn tick_info() {
        ...
        let body_bytes = response.into_body().collect().await.unwrap().to_bytes();
        let actual: TickInfoWrapper = serde_json::from_slice(&body_bytes).unwrap();
