use crate::qubic_types::{
    ...
    MiningSeed, Nonce, QubicId, QubicTxHash, QubicWallet, Signature,
};
...
use crate::qubic_tcp_types::{
    consts::NUMBER_OF_TRANSACTION_PER_TICK, utils::QubicRequest, MessageType,
};

use super::{
    assets::{IssueAssetInput, TransferAssetInput, ISSUE_ASSET_FEE, QXID, TRANSFER_FEE},
    send_to_many::{SendToManyInput, SEND_TO_MANY_CONTRACT_INDEX},
    ContractIpoBid,
};
...
impl ToBytes for TransactionData {
    fn to_bytes(&self) -> Vec<u8> {
        match self {
            ...
            TransactionData::SubmitWork { seed, nonce } => {
                [seed.to_bytes(), nonce.to_bytes()].concat()
            }
    ...
    pub fn sanitize_transaction(&self, tx: &mut RawTransaction) {
        match self {
            ...
            Self::SubmitWork { .. } => {
                ...
                tx.amount = 1_000_000;
                tx.input_type = 2;
                tx.input_size = (core::mem::size_of::<MiningSeed>() as u16)
                    + (core::mem::size_of::<Nonce>() as u16);
            }
...
impl FromBytes for TransactionWithData {
    fn from_bytes(data: &[u8]) -> Result<Self, crate::qubic_types::errors::ByteEncodingError> {
        if data.len() < core::mem::size_of::<RawTransaction>() + core::mem::size_of::<Signature>() {
            return Err(
                crate::qubic_types::errors::ByteEncodingError::InvalidMinimumDataLength {
                    expected_min: core::mem::size_of::<RawTransaction>()
        ...
        match raw_tx.input_type {
            ...
            1 => {
                if raw_tx.input_size as usize == core::mem::size_of::<IssueAssetInput>() {
                    let input =
                        unsafe { read_unaligned(tx_data.as_ptr() as *const IssueAssetInput) };

                    data = TransactionData::IssueAsset(input);
                } else if raw_tx.input_size as usize == core::mem::size_of::<SendToManyInput>() {
                    let input =
                        unsafe { read_unaligned(tx_data.as_ptr() as *const SendToManyInput) };
            ...
            2 => {
                if raw_tx.input_size as usize
                    == (core::mem::size_of::<MiningSeed>() + core::mem::size_of::<Nonce>())
                    && tx_data.len()
                        == (core::mem::size_of::<MiningSeed>() + core::mem::size_of::<Nonce>())
                    && raw_tx.amount == 1_000_000
                {
                    data = TransactionData::SubmitWork {
                        seed: MiningSeed::from_bytes(
                            &tx_data[..core::mem::size_of::<MiningSeed>()],
                        )?,
                    ...
                    let input =
                        unsafe { read_unaligned(tx_data.as_ptr() as *const TransferAssetInput) };
